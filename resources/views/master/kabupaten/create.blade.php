...
<div class="container-fluid">
    <div class="row">
        <form id="formInput" class="col-md-12" method="post" action="{{ ($data['typeForm'] == 'create') ? route('master.kabupaten.store') : route('master.kabupaten.update',$data['dataModel']->KabID) }}" enctype="multipart/form-data" autocomplete="off">
            ...
            <h1 class="p-0">Master Kabupaten</h1>
            <div class="card">
                <div class="card-header">
                    <div class="d-flex flex-row justify-content-between">
                        <div>Form Master Kabupaten</div>
                        <div class="btn-group">
                    ...
                    <div class="row">
                        <div class="col-md-6 col-sm-12">
                            ...
                            <div class="form-group row">
                                <label for="jenistest" class="form-label col-md-3">Provinsi <span class="text-danger">*</span> :</label>
                                <div class="col-md-8">
                                    <select name="provinsi" class="form-control select2" required>
                                        @foreach($data['Provinsi'] as $item)
                                             <option value="">Pilih Provinsi </option>
                                        @if ($data['typeForm'] == 'create')
                                                <option value="{{ $item->ProvID }}"> {{ $item->Provinsi }} </option>
                                            @else
                                                <option {{ ($data['dataModel']->Provinsi == $item->ProvID) ? "selected" : "" }} value="{{ $item->ProvID }}"> {{ $item->Provinsi }} </option>
                                            @endif
                                        @endforeach
                                    </select>
                                </div>
                            ...
                            <div class="form-group row">
                                <label for="no_pp" class="form-label col-md-3">Kabupaten <span class="text-danger">*</span> :</label>
                                <div class="col-md-8">
                                    <input type="text" class="form-control" name="kabupaten"
                                    value="{{ ($data['typeForm'] == 'create') ? "" : $data['dataModel']->Kabupaten }}"  required>
                                </div>
                            </div>
                        </div>
